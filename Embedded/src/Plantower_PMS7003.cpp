...
void Plantower_PMS7003::updateFrame() {
  if (!initialized) {
    debug("Error: must call Plantower_PMS7003::init()");
    return;
  }
  dataReady = false;
  if (serial->available()) {
    ...
    if (nextByte == 0x4d && lastByte == 0x42 ) {
      ...
      bufferIndex=1;
      debug("Start byte found");
    }
    sensorData.bytes[bufferIndex] = nextByte;    
    bufferIndex++;
    lastByte = nextByte;
  } else {
      debug("Serial is not available");
  }
  ...
  if (bufferIndex == PMS7003_DATA_SIZE) {
    if (sensorData.bytes[0] == 0x42 && sensorData.bytes[1] == 0x4d) {
      ...
      if(isValidChecksum()) {
        ...
        if (debugMode) {
          debug("Invalid data checksum");
        }
      }
    } else {
      if (debugMode) {
        debug("Malformed first byte");
      }
    }
    bufferIndex=0;
  }   
...
void Plantower_PMS7003::dumpBytes() {
  for(int i=0; i<PMS7003_DATA_SIZE; i++) {
    debug(String(sensorData.bytes[i], HEX));
    debug(" ");
  }
  debugPrint("");
}
